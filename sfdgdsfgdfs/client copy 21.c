    ...
    while (1){
        ...
        if ((side == 'B' && price > *input_best_bid) || (side == 'A' && price < *input_best_ask)) {
            if (quantity > 0) {
                ...
                //printf("%s Best_Bid=%.4f, Best_Ask=%.4f\n", ip_address, *input_best_bid, 1234.0);
            }
        }
        // printf("자식 종료\n");
        close(sockfd); // 연결 종료
...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        for (int i = 1; i < argc; ++i) {
            ...
            if (pid == -1) {
                ...
                if (RUN_FIRST) {
                    ...
                    handle_connection(argv[i], pipes[i - 1], NULL, &best_ask);
                }
                else {
                    double bid, ask;
        ...
        for (int i = 1; i < argc; ++i) {
            ...
            // printf("[Current] Bid=%.4f, Ask=%.4f\n", bid, ask);
            // printf("[Best] Bid=%.4f, Ask=%.4f\n", max_bid, min_ask);
        }
