...
void handle_connection(const char *ip_address, int pipe_fd[], double *max_bid, double *min_ask) {
    ...
    while (1){
        ...
        if ((side == 'B' && price > *max_bid) || (side == 'A' && price < *min_ask)) { 
            if (quantity > 0) {
                if (side == 'B') {
                    *max_bid = price;
                } 
...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        // 부모 프로세스
        double max_bids[MAX_SERVERS] = {0.0}; // 각 서버의 최대값을 저장할 배열 초기화
        double min_asks[MAX_SERVERS] = {DBL_MAX}; // 각 서버의 최소값을 저장할 배열 초기화
        ...
        for (int i = 1; i < argc; ++i) {
            ...
            if (pid == -1) {
                ...
                if (RUN_FIRST) {
                    ...
                    handle_connection(argv[i], pipes[i - 1], &max_bids[i - 1], &min_asks[i - 1]);
                }
                else {
                    double bid, ask;
