...
int main(int argc, char *argv[]) {
    ...
    for (int i = 1; i < argc; ++i) {
            // server_infos[i - 1].max_bid = N_INFINITY;
            //server_infos[i - 1].min_ask = P_INFINITY; 
            server_infos[i - 1].max_bid = 0.0;
            server_infos[i - 1].min_ask = 9999.0; 
    }
    ...
    while (1) {
        ...
        for (int i = 1; i < argc; ++i) {
            ...
            // printf("%.4f, %.4f |%.4f, %.4f   \n",server_infos[i - 1].max_bid , bid, server_infos[i - 1].min_ask , ask);
            // printf("%s ", server_infos[i - 1].max_bid > bid ? "true" : "false");
            // printf("%s \n", server_infos[i - 1].min_ask < ask ? "true" : "false");
            // printf(server_infos[i - 1].max_bid > bid)

            if (server_infos[i - 1].max_bid < bid && bid !=0){
                server_infos[i - 1].max_bid = bid;
            }

            if (server_infos[i - 1].min_ask > ask && ask !=0){
        ...
        double p_ask = 9999;
        for (int i = 1; i < argc; ++i) {
            // if (server_infos[i - 1].max_bid > p_bid){
            //             server_infos[i - 1].max_bid = p_bid;
            //         }

            // if (server_infos[i - 1].min_ask < p_ask){
            //             server_infos[i - 1].min_ask = p_ask;
            //         }

            if (server_infos[i - 1].max_bid < p_bid && p_bid !=0){
                server_infos[i - 1].max_bid = p_bid;
            }

            if (server_infos[i - 1].min_ask > p_ask && p_ask !=0){
                server_infos[i - 1].min_ask = p_ask;
            }
        }
