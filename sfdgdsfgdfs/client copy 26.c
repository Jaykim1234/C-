...
void handle_connection(const char *ip_address, int pipe_fd[], double *max_bid, double *min_ask) {
    int sockfd;
    while (1){
        ...
        if ((side == 'B' && price > server_info->max_bid) || (side == 'A' && price < server_info->min_ask)) { 
            if (quantity > 0) {
                if (side == 'B') {
                    server_info->max_bid = price;
                } 
                else {
                    server_info->min_ask = price;
                }

                // 데이터를 부모 프로세스로 전송
                char combined_str[2 * sizeof(double) + 2];  // 최대 크기로 지정
                // sprintf(combined_str, "%.4f,%.4f", *input_best_bid, *input_best_ask);
                sprintf(combined_str, "%.4f,%.4f", server_info->max_bid, server_info->min_ask);
                write(pipe_fd[1], combined_str, strlen(combined_str) + 1);  // Null 문자까지 포함
                printf("%s Best_Bid=%.4f, Best_Ask=%.4f\n", ip_address, server_info->max_bid, server_info->min_ask);
            }
        }
        close(sockfd); // 연결 종료
        break;
...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        for (int i = 1; i < argc; ++i) {
            ...
            ServerInfo server_info = {0.0, DBL_MAX}; // Initialize max_bid and min_ask
            ...
            if (pid == -1) {
                ...
                if (RUN_FIRST) {
                    ...
                    handle_connection(argv[i], pipes[i - 1], &server_info);
                }
                else {
                    double bid, ask;
                    ...
                    server_info.max_bid = bid;
                    server_info.min_ask = ask;
                    handle_connection(argv[i], pipes[i - 1], &server_info);
                }

                // 각 서버 정보 배열에 저장
                server_infos[i - 1] = server_info;
