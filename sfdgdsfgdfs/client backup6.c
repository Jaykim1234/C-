    ...
    while (1){
        ...
        if ((side == 'B' && price > server_infos[server_index].max_bid) || (side == 'A' && price < server_infos[server_index].min_ask)) { 
            if (quantity > 0) {
                ...
                sprintf(combined_str, "%.4f,%.4f,%d", server_infos[server_index].max_bid, server_infos[server_index].min_ask, server_index);
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 1; i < argc; ++i) {
            ...
            server_infos[i - 1].max_bid = 0;
            server_infos[i - 1].min_ask = 9999; 
    }
    ...
    while (1) {
        ...
        for (int i = 1; i < argc; ++i) {
            ...
            if (pid == -1) {
                ...
                if (RUN_FIRST) {
                    ...
                    sscanf(received_data[i - 1], "%lf,%lf,%lf", &bid, &ask, &server_index2); // 데이터를 콤마로 분리하여 best_bid와 best_ask 값을 얻음
        ...
        for (int i = 1; i < argc; ++i) {
            ...
            double bid, ask, server_index_p;
            // sscanf(received_data[i - 1], "%lf,%lf", &bid, &ask);
            sscanf(received_data[i - 1], "%lf,%lf,%lf", &bid, &ask, &server_index_p);
        ...
        printf("Best Bid=%.4f, Best Ask=%.4f ", max_bid, min_ask);
        printf("-------------------------------------\n");
        RUN_FIRST = false;
    }
